...
export async function getLabMonitorData(): Promise<LabMonitorData> {
  ...
  try {
    ...
    const [patientsResult, salesResult] = await Promise.all([
      ...
      saleIds.length ? supabase.from("ventas").select("id,folio,fecha_entrega_estimada").in("id", saleIds) : Promise.resolve({ data: [], error: null }),
    ]);
    if (patientsResult.error || salesResult.error) return { status: "error", message: "Las órdenes cargaron, pero faltan datos de pacientes o ventas.", ...empty };
