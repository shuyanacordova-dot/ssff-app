...
export type TaskData = { status: "ready" | "needs_login" | "forbidden" | "error"; message?: string; profile?: { id: string; nombre: string; rol: string; empresaId: string; sucursalId: string }; tasks: TaskRecord[]; supervisorTaskIds: string[]; team: TeamMember[]; notifications: TaskNotification[] };
...
export async function getTaskData(): Promise<TaskData> {
  ...
  try {
    ...
    return { status: "ready", message: teamError ? "No se pudo cargar el equipo para asignar tareas." : undefined, profile: { id: profile.id, nombre: profile.nombre, rol, empresaId: profile.empresa_id, sucursalId: profile.sucursal_id }, tasks, supervisorTaskIds: (supervisionRows ?? []).map((row) => row.tarea_id), team: (teamRows ?? []) as TeamMember[], notifications: (notificationsResult.data ?? []) as TaskNotification[] };
