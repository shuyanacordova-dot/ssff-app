...
export type CuentasCobrarData = { status: "ready" | "needs_configuration" | "needs_login" | "forbidden" | "error"; message?: string; profile?: CuentasCobrarProfile; deudas: DeudaPaciente[]; empresasConvenio: EmpresaConvenio[] };
...
export async function getCuentasCobrarData(): Promise<CuentasCobrarData> {
  const empty = { deudas: [], empresasConvenio: [] };
  ...
  try {
    ...
    const [ventasResult, empresasConvenioResult] = await Promise.all([
      supabase.from("ventas").select("id,empresa_id,paciente_id,total,pagado,saldo,fecha_entrega_estimada,creado_en,empresas(nombre)").eq("estado", "completada").gt("saldo", 0).not("paciente_id", "is", null).order("creado_en", { ascending: true }),
    ...
    if (!pacienteIds.length) return { status: "ready", profile: { id: profile.id, empresa_id: profile.empresa_id, rol: role }, deudas: [], empresasConvenio };
    ...
    return { status: "ready", profile: { id: profile.id, empresa_id: profile.empresa_id, rol: role }, deudas: Array.from(grupos.values()).sort((a, b) => b.saldo_total - a.saldo_total), empresasConvenio };
  } catch {
