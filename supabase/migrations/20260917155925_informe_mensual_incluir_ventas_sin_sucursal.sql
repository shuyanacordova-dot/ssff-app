...
declare v_user uuid; v_role text; v_desde date; v_hasta date; v_desde_ts timestamptz; v_hasta_ts timestamptz;
        v_por_sucursal jsonb; v_sin_sucursal jsonb; v_gastos_clasif jsonb; v_cobros_metodo jsonb; v_cuadres jsonb; v_cxc numeric; v_totales jsonb; v_meta_total numeric; v_gastos_total numeric;
begin
  ...
  -- Ventas completadas sin sucursal asignada (para que el reporte cuadre con el total del mes)
  select case when count(*) = 0 then '[]'::jsonb else jsonb_build_array(jsonb_build_object(
      ...
      'ventas_total', coalesce(sum(total), 0), 'ventas_count', count(*), 'cobrado_total', coalesce(sum(pagado), 0), 'meta', 0
    )) end
    into v_sin_sucursal
  from public.ventas
