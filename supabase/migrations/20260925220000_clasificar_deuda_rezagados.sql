-- La clasificación manual de deudas acepta también "Lentes rezagados" (lentes listos que el paciente no retira,
-- p. ej. de Optox, sin orden de laboratorio en LumOS). "Apartados" se maneja con ventas.apartado.
...
alter table public.pacientes_clinicos add constraint pacientes_clinicos_categoria_cobro_check
  check (categoria_cobro is null or categoria_cobro in ('urgentes','recientes','mensuales','convenio','rezagados'));
...
begin
  ...
  if p_categoria is not null and p_categoria not in ('urgentes','recientes','mensuales','convenio','rezagados') then
