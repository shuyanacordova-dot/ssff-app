...
export default function InformesBoard(props: InformesData) {
  ...
  if (props.status !== "ready") return <main className="page agenda-page"><div className="container agenda-shell"><header className="agenda-header"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">DIRECCIÓN</p><h1>Informes</h1><p className="subtitle">{props.message ?? "No se pudo abrir informes."}</p></div>{props.status === "needs_login" && <Link className="primary-link" href="/login?next=/informes">Iniciar sesión</Link>}</header></div></main>;
  ...
  return <main className="page agenda-page"><div className="container agenda-shell">
    <header className="agenda-header no-print"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">DIRECCIÓN</p><h1>Informes</h1><p className="subtitle">Metas, ventas por sucursal e informe mensual del negocio.</p></div>
    ...
    {loading && !informe ? <p className="field-hint">Calculando informe…</p> : informe && <>
      <section className="agenda-summary">
        ...
        <article><Wallet size={21} /><strong>{money(informe.totales.cobrado_total)}</strong><span>cobrado del mes</span></article>
...
function SucursalRow({ row, showEmpresa, onGuardarMeta }: { row: InformeSucursal; showEmpresa: boolean; onGuardarMeta: (monto: number) => void }) {
  ...
  return <article className="task-card"><div className="task-status"><span className={`status-dot ${row.meta > 0 && row.ventas_total >= row.meta ? "aprobada" : ""}`} /></div><div className="task-main">
    ...
    <p>Ventas: <strong>{money(row.ventas_total)}</strong> · Cobrado: {money(row.cobrado_total)}</p>
    {row.meta > 0 && <>
