...
export default function PatientClinicalClient(props: ClinicalData & { autoCreate?: boolean; initialSearch?: string; initialPacienteId?: string; initialVentaId?: string }) {
  ...
  return <main className="page clinical-page"><div className="container clinical-shell">
    ...
    <section className="clinical-main">{selected ? <>
      <article className="glass patient-hero"><div className="patient-identity"><span className="patient-avatar large">{initials(selected)}</span><div><p className="section-label">CARPETA CENTRAL DEL PACIENTE</p><h2>{fullName(selected)}</h2><p>{selected.cedula ?? "Sin cédula"} · {selected.telefono || "Sin WhatsApp"}</p>{saldoFavorTotal > 0.004 && <p className="origin-badge" style={{ display: "inline-block", marginTop: 6 }}>Saldo a favor: ${saldoFavorTotal.toFixed(2)}</p>}</div></div>{!demoMode && canEditPatient && <button className="outline-action" type="button" onClick={openEditing}><Pencil size={16} /> Editar datos</button>}{!demoMode && canAuthorClinical && <button className="new-consultation" type="button" onClick={() => setIsConsulting(true)}><ClipboardPlus size={17} /> Nueva revisión</button>}</article>
