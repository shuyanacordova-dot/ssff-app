...
import { AlertTriangle, ArrowLeft, Building2, CalendarDays, ChevronRight, ClipboardPlus, Image as ImageIcon, History, Plus, ReceiptText, Search, ShieldCheck, Stethoscope, X } from "lucide-react";
...
export default function PatientClinicalClient(props: ClinicalData & { autoCreate?: boolean }) {
  ...
  const selectPatient = (id: string) => {
    setSelectedId(id); setSection("consultations"); setHistoryBranchId("all");
  ...
  return <main className="page clinical-page"><div className="container clinical-shell">
    ...
    <section className="clinical-main">{selected ? <>
      <article className="glass patient-hero"><div className="patient-identity"><span className="patient-avatar large">{initials(selected)}</span><div><p className="section-label">CARPETA CENTRAL DEL PACIENTE</p><h2>{fullName(selected)}</h2><p>{selected.cedula ?? "Sin cédula"} · {selected.telefono || "Sin WhatsApp"}</p></div></div>{!demoMode && canAuthorClinical && <button className="new-consultation" type="button" onClick={() => setIsConsulting(true)}><ClipboardPlus size={17} /> Nueva revisión</button>}</article>
