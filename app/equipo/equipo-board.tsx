...
export default function EquipoBoard(props: EquipoData) {
  ...
  if (props.status !== "ready") return <main className="page agenda-page"><div className="container agenda-shell"><header className="agenda-header"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">CONFIGURACIÓN</p><h1>Equipo y accesos</h1><p className="subtitle">{props.message ?? "No se pudo abrir el equipo."}</p></div>{props.status === "needs_login" && <Link className="primary-link" href="/login?next=/equipo">Iniciar sesión</Link>}</header></div></main>;
