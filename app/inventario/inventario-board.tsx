...
export default function InventarioBoard(props: InventarioData & { grupoInicial?: "monturas" | "lunas" }) {
  ...
  return <main className="page agenda-page"><div className="container agenda-shell">
    <header className="agenda-header"><div><Link className="back-link" href="/">← LUMOS</Link><p className="eyebrow">OPERACIÓN</p><h1>{titulo}</h1><p className="subtitle">{subtitulo}</p></div><div className="tabs">{props.companies.map((c) => <button key={c.id} className={c.id === empresaId ? "active" : ""} onClick={() => { setEmpresaId(c.id); setMarcaActiva(""); }}>{c.nombre}</button>)}</div></header>
