...
export default function InventarioBoard(props: InventarioData & { grupoInicial?: "monturas" | "lunas" }) {
  ...
  if (props.status !== "ready") return <main className="page agenda-page"><div className="container agenda-shell"><header className="agenda-header"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">OPERACIÓN</p><h1>Inventario</h1><p className="subtitle">{props.message ?? "No se pudo abrir inventario."}</p></div>{props.status === "needs_login" && <Link className="primary-link" href="/login?next=/inventario">Iniciar sesión</Link>}</header></div></main>;
  ...
  const productosCategoria = clasificacionActiva ? productosCategoriaSinFiltrar.filter((p) => p.clasificacion === clasificacionActiva) : productosCategoriaSinFiltrar;
  ...
  return <main className="page agenda-page"><div className="container agenda-shell">
    <header className="agenda-header"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">OPERACIÓN</p><h1>{titulo}</h1><p className="subtitle">{subtitulo}</p></div><div className="tabs">{props.companies.map((c) => <button key={c.id} className={c.id === empresaId ? "active" : ""} onClick={() => setEmpresaId(c.id)}>{c.nombre}</button>)}</div></header>
