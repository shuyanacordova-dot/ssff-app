...
export default function Cart({ products, stock, branches, patients, empresasConvenio, defaultCompany, defaultBranch, defaultPacienteId, lockPatient, onDone, saldosFavor = {} }: { saldosFavor?: Record<string, number>; products: SaleProduct[]; stock: SaleStock[]; companies: SaleCompany[]; branches: SaleBranch[]; patients: SalePatient[]; empresasConvenio: EmpresaConvenio[]; defaultCompany: string; defaultBranch: string; defaultPacienteId?: string; lockPatient?: boolean; onDone: (message: string) => void }) {
  ...
  const precioItem = (item: CartItem) => { const product = available.find((p) => p.id === item.producto_id); if (!product) return 0; if (precioEditable(product) && item.precio !== undefined) { const n = Number(item.precio.replace(",", ".")); return Number.isFinite(n) ? n : 0; } return Number(product.precio_venta || 0); };
  ...
  const addProduct = (id: string) => { if (id && available.some((product) => product.id === id) && !items.some((item) => item.producto_id === id)) setItems([...items, { producto_id: id, cantidad: 1, descuento: 0, ...(precioEditable(available.find((p) => p.id === id)) ? { precio: String(Number(available.find((p) => p.id === id)?.precio_venta || 0)) } : {}) }]); };
  ...
  const submit = (form: HTMLFormElement) => {
    ...
    start(async () => {
      ...
      data.set("items", JSON.stringify(items.map((item) => ({ producto_id: item.producto_id, cantidad: item.cantidad, descuento: item.descuento, ...(item.precio !== undefined ? { precio_unitario: precioItem(item) } : {}) }))));
