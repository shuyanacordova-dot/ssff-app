...
export default function ResumenDiaBoard(props: ResumenDiaData & { fecha: string }) {
  ...
  if (props.status !== "ready") return <main className="page agenda-page"><div className="container agenda-shell"><header className="agenda-header"><div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">OPERACIÓN DEL DÍA</p><h1>Resumen del día</h1><p className="subtitle">{props.message ?? "No se pudo abrir el resumen."}</p></div>{props.status === "needs_login" && <Link className="primary-link" href="/login?next=/resumen-dia">Iniciar sesión</Link>}</header></div></main>;
  ...
  return <main className="page agenda-page"><div className="container agenda-shell">
    <header className="agenda-header no-print">
      <div><Link className="back-link" href="/">← SHUVISION OS</Link><p className="eyebrow">OPERACIÓN DEL DÍA</p><h1>Resumen del día</h1><p className="subtitle">Ventas, abonos y salidas del día — la base para cerrar la <Link href="/caja" className="text-action" style={{ display: "inline" }}>caja diaria</Link>.</p></div>
