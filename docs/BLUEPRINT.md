...
| Inventario | ✅ | Stock por sucursal, transferencias, alertas, búsqueda por marca/modelo/código. **Banco de lunas** (`/inventario/lunas`): lunas de bodega y de garantía por sucursal; la orden de laboratorio avisa "Tienes esta luna en bodega / de garantía" y permite usarla (descuenta y deja registro). **Hacer inventario** (`/inventario/conteo`): conteo rápido por clasificación y detalle uno por uno donde no cuadra; el ajuste solo se aplica al aprobarlo (Shuyana en todas, Joao en Sacha, Erick en Focus) |
...
| L29 | 2026-09-26 | Una preferencia guardada en el navegador y compartida entre pantallas ("Todas las sucursales") hizo que el Resumen del día y la Caja mezclaran Shuvision con Sacha después de marcarla en Cuentas por cobrar. | Caja y resumen son por sucursal: se ven de una sola sucursal (botones por sucursal), sin opción de mezclar. Una preferencia de "ver todo" no se comparte entre pantallas. |
...
| 2026-09-25 | Mis deudas: cargadas 16 deudas del Excel "DEUDAS SHU Y SHUVISION TARJETAS" (12 de Shuvisión $52,437.56 + 4 personales de Shu $8,282.76; Noon no se carga por estar pagado). Nuevo ámbito **personal** ("Personal Shu"), filtro Todo/Óptica/Personal, botón **Editar**, notas visibles y "día por confirmar" (sin día de pago se usa el fin de mes). Pendientes de Shuyana: día de pago de Pichincha y BanEcuador, saldo real del Crédito Jassy, cuotas de BanEcuador, confirmar que American Gold = American Business, deuda Mastercard $200 del 24-ago. Migración `20260926030000_mis_deudas_carga_excel.sql`. | (este commit) |
| 2026-09-25 | **Rendimiento 2 y 3:** usuario de la sesión y contexto operativo se consultan una sola vez por carga de página (`lib/supabase/current-user.ts`, `getOperationalContext` con `cache`). El catálogo (1.349 productos) y el stock ya no se descargan al abrir Ventas ni la carpeta del paciente: se cargan bajo demanda al abrir una venta, orden de laboratorio o garantía (`app/ventas/catalogo-actions.ts`, `use-catalogo-venta.ts`). Mis deudas: día de pago Pichincha (Jassy 1, Shu 15) y 3 pagos atrasados de tarjetas (American Gold $150, Diners $111.28 al 21-sep, Mastercard $200 al 16-sep). | (este commit) |
| 2026-09-26 | **Privacidad por sucursal:** Resumen del día ahora solo trae (desde el servidor) las sucursales a las que la persona tiene acceso, con casilla "Todas las sucursales" para quien tiene varias; en la carpeta de un paciente compartido cada persona ve solo las ventas de su sucursal (las revisiones siguen compartidas). Caso: Tiffany (Sacha) veía la venta de Judith Oña hecha en Shuvision. | (este commit) |
...
| 2026-09-26 | Mensaje del recibo virtual con presentación: "Hola {primer nombre} 👋, te saludamos de ShuVision Óptica / Focus Óptica (según la empresa de la venta). ¡Gracias por confiar en nosotros! 🌿 Cuidemos el medio ambiente… ticket virtual + enlace". `lib/mensajes.ts` (`mensajeTicketVirtual`). | (este commit) |
...
| B4 | Inventario inicial contado por sucursal | El stock debe partir de un conteo real | 🟡 herramienta lista (`/inventario/conteo`), falta hacer el conteo en cada sucursal |
